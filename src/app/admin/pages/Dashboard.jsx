...
const Dashboard = () => {
  ...
  const fetchAllStats = async () => {
    try {
      ...
      const selectedDateInvoices =
        invoicesList?.filter((inv) => {
          const invDate = new Date(
            inv.invoiceDate?.toDate?.() || inv.invoiceDate,
          );
          return invDate.toDateString() === selectedDateStr;
        }) || [];

      // Calculate daily sales for selected date
      const dailySalesSelected =
        selectedDateInvoices.reduce(
      ...
      const monthlySales =
        monthlyInvoices.reduce((sum, inv) => sum + (inv.finalAmount || 0), 0) ||
        0;
      ...
      const hourlyData = Array.from({ length: 24 }, (_, i) => ({
        ...
        revenue: selectedDateVisits
          .filter(
            (v) => new Date(v.date?.toDate?.() || v.date).getHours() === i,
          )
      ...
      // Customer analytics
      const uniqueCustomersSelected = new Set(
        selectedDateVisits.map((v) => v.customerId),
      ).size;

      // Revenue distribution by hour
      const avgOrderValue =
        selectedDateInvoices.length > 0
          ? dailySalesSelected / selectedDateInvoices.length
          : 0;
      ...
      visitsList?.forEach((v) => {
        customerVisitCount[v.customerId] =
          (customerVisitCount[v.customerId] || 0) + 1;
      });
      const returningCustomers = Object.values(customerVisitCount).filter(
        (count) => count > 1,
      ).length;
      ...
      setStats({
        ...
        conversionRate: (
          (uniqueCustomersSelected / (customerList?.length || 1)) *
          100
        ).toFixed(1),
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            Welcome to Velvet Luxury Salon
          </p>
          <p style={{ fontSize: "0.875rem", opacity: 0.85 }}>
            Here's your real-time business intelligence dashboard
          </p>
        </div>
      ...
      <div className="card-grid" style={{ marginBottom: "2.5rem" }}>
        <StatCard
          title="Daily Sales"
          value={`₹${stats.dailySales.toFixed(0)}`}
          icon={IndianRupee}
          color="#d4af37"
        />
        <StatCard
          title="Monthly Revenue"
          value={`₹${stats.monthlySales.toFixed(0)}`}
          icon={TrendingUp}
          color="#10b981"
        />
        <StatCard
      ...
      <div className="layout-two-col" style={{ marginBottom: "2.5rem" }}>
        {/* Charts Container */}
        <div>
                    ...
                    {stats.topServices.slice(0, 5).map((service, idx) => (
                      <li
                        ...
                        style={{
                          padding: "0.75rem 0",
                          borderBottom:
                            idx < stats.topServices.length - 1
                    ...
                    {stats.topProducts.slice(0, 5).map((product, idx) => (
                      <li
                        ...
                        style={{
                          padding: "0.75rem 0",
                          borderBottom:
                            idx < stats.topProducts.length - 1
                              ? "1px solid var(--glass-border)"
                              : "none",
                          display: "flex",
                          justifyContent: "space-between",
      ...
      <div className="card">
        <div className="card-header">
          <h2 className="card-title">
            Latest Transactions - {selectedDate.toLocaleDateString()}
          </h2>
        </div>
        <div
          className="card-content"
