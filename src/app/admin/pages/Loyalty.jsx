...
const Loyalty = () => {
  ...
  const [spinPoints, setSpinPoints] = useState(100);
  const [awardingSpins, setAwardingSpins] = useState(false);
  ...
  const handleShuffleAndSelect = () => {
    ...
    if (count > customers.length) {
      setError(
        `Cannot select ${count} customers. Only ${customers.length} customers available.`,
      );
      return;
    }
    ...
    setTimeout(() => {
      ...
      setSuccess(
        `Randomly selected ${count} customer${count !== 1 ? "s" : ""} for gift!`,
      );
  ...
  const handleAwardSpin = async () => {
    if (!selectedCustomer || !spinPoints) {
      setError("Please select a customer and enter points");
      return;
    }

    try {
      setAwardingSpins(true);
      setError("");
      setSuccess("");

      const points = parseInt(spinPoints) || 0;
      if (points <= 0 || points > 5000) {
        setError("Points must be between 1 and 5000");
        setAwardingSpins(false);
        return;
      }

      const result = await awardCustomerSpin(
        selectedCustomer.id,
      ...
      if (result.success) {
        setSuccess(
          `Successfully awarded ${points} points to ${selectedCustomer.name}!`,
        );
        setLoyaltyPoints(loyaltyPoints + points);
        setSpinPoints(100);

        // Reload customer data
        const updatedCustomers = await getCustomers(false);
        setCustomers(updatedCustomers);

        setTimeout(() => setSuccess(""), 5000);
      }
    } catch (err) {
      console.error("Error awarding spin:", err);
      setError(err.message || "Failed to award spin");
    } finally {
      setAwardingSpins(false);
    }
  };
  ...
  return (
    <div>
      ...
      {showShuffleModal && (
        <div
          className="shuffle-modal-overlay"
                ...
                >
                  ...
                  <div
                    style={{
                      padding: "1rem",
                      background:
                        "linear-gradient(135deg, rgba(232, 197, 71, 0.1) 0%, rgba(201, 162, 39, 0.05) 100%)",
                      borderRadius: "var(--radius-sm)",
                  ...
                  >
                    ...
                    >
                      {Math.floor(loyaltyPoints / 20)} available as rupees
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      ₹{(selectedCustomer.calculatedTotalSpent || 0).toFixed(2)}
                    </div>
                    <div
                      style={{
                        fontSize: "0.75rem",
                    ...
                    >
                      {Math.floor(
                        (selectedCustomer.calculatedTotalSpent || 0) * 1,
                      )}{" "}
                  ...
                  >
                    ...
                    >
                      {selectedCustomer.totalVisits || customerVisits.length}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      ...
                      {customerVisits.length > 0
                        ? (
                            (selectedCustomer.calculatedTotalSpent || 0) /
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        max="5000"
                        value={spinPoints}
                        onChange={(e) => setSpinPoints(e.target.value)}
                        placeholder="Enter points (1-5000)"
                        disabled={awardingSpins}
                        style={{
                          flex: 1,
                          padding: "0.75rem",
                          border: "1px solid var(--border)",
                      ...
                      />
                      <button
                        onClick={handleAwardSpin}
                        disabled={awardingSpins}
                        style={{
                          padding: "0.75rem 1.5rem",
                          background: awardingSpins
                            ? "var(--muted)"
                          ...
                          borderRadius: "var(--radius-sm)",
                          fontWeight: 600,
                          cursor: awardingSpins ? "not-allowed" : "pointer",
                          fontSize: "0.95rem",
                          display: "flex",
                          alignItems: "center",
                          gap: "0.5rem",
                    ...
                    >
                      Award loyalty points from the spin wheel to this customer.
                      Points will be added to their loyalty account immediately.
                    </div>
                    <div
                      style={{
                        padding: "0.75rem",
                    ...
                    >
                      <strong>⚠️ Development Notice:</strong> The Spin Wheel
                      feature is under development. Points awarded here are
                      provisional and will be reviewed by the reception team.
                      These points may not be valid until development is
                      complete.
                    </div>
                  </div>
                )}
                      ...
                      {pointsHistory.slice(0, 10).map((tx) => (
                        ...
                        >
                          <div>
                            ...
                            >
                              {tx.description ||
                                `${tx.type === "earned" ? "Points Earned" : tx.type === "deducted" ? "Points Used" : "Points Adjusted"}`}
                            </div>
                            <div
                              style={{
                                fontSize: "0.75rem",
                      ...
                      {customerVisits.map((visit, idx) => (
                        ...
                        >
                          ...
                          {visit.items && visit.items.length > 0 && (
                            ...
                            >
                              {visit.items.length} service
                              {visit.items.length !== 1 ? "s" : ""}
                            </div>
                          )}
                        </div>
                      ))}
