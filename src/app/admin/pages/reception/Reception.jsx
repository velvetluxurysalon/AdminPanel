...
const ReceptionComponent = () => {
  ...
  console.log('📲 Filtered visits for active section', uiState.activeSection, ':', filteredVisits);
      ...
      <div style={{ width: '100%', animation: 'fadeIn 0.5s ease-out' }}>
        ...
        {birthdayCustomers.length > 0 && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <div>
                <div style={{ fontWeight: '700', color: '#ec4899', fontSize: '1rem' }}>
                  🎉 Birthday Today!
                </div>
                <div style={{ color: '#6b7280', fontSize: '0.875rem', marginTop: '0.25rem' }}>
                  {birthdayCustomers.map(c => c.name).join(', ')} {birthdayCustomers.length > 1 ? 'are' : 'is'} celebrating today!
                </div>
              </div>
            </div>
            <button
              onClick={() => setDismissedBirthdays([...dismissedBirthdays, ...birthdayCustomers.map(c => c.id)])}
              style={{
        ...
        <div style={{ marginTop: '1.5rem' }}>
          ...
          <VisitsTable
            ...
            onDeleteVisit={(visitId) => handleDeleteVisit(visitId, dataState.fetchAllData, dataState.setSuccess, dataState.setError)}
