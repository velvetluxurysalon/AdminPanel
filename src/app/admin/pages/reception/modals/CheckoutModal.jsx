...
const CheckoutModal = ({
  ...
  const [globalPointsSettings, setGlobalPointsSettings] = useState(null);
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      try {
        ...
        setGlobalPointsSettings(settings);
  ...
  if (discountType === "coupon" && couponData) {
    ...
    // Dynamically use global redemption rate or fallback to 20 pts = 1 rupee
    const redemptionRate = globalPointsSettings?.redemptionRate || 20;
    const points = parseFloat(coinsUsed) || 0;
    coinsDiscount = points / redemptionRate;
    finalDiscount = coinsDiscount;
    discountDescription = `Loyalty Points Discount (rate: ${redemptionRate} pts/₹)`;
  }
  ...
  const handleCompletePayment = async () => {
    try {
      ...
      // Use actual redemption rate from global settings
      const redemptionRate = globalPointsSettings?.redemptionRate || 20;
      ...
      // Award points only for amount paid OUT OF POCKET (not from discount)
      const pointsDiscountRupees = pointsDeducted / redemptionRate;

      // Fetch global points settings
      let ptsPerRupee = 1.0;
      if (globalPointsSettings?.pointsPerRupee) {
        ptsPerRupee = parseFloat(globalPointsSettings.pointsPerRupee);
      }

      pointsEarned = Math.floor(
        Math.max(0, (amountPaidValue - pointsDiscountRupees) * ptsPerRupee),
      );
      ...
      console.log("📊 [Points Update Debug]", {
        ...
        discountType,
        customerId,
        calculation: `${currentPoints} - ${pointsDeducted} + ${pointsEarned} = ${finalLoyaltyPoints}`,
      });

      // Generate formatted invoice ID (VELVET0001 format)
      const invoiceId = await generateInvoiceId();

      // --- Referrer Logic (A refers B) ---
      try {
        const normalizedPhone = latestCustomer.phone || customerId;
        const pendingReferral = await getReferralForReferee(normalizedPhone);

        if (pendingReferral) {
          console.log(
            "🎁 Founding pending referral for this customer:",
            pendingReferral,
          );

          // Match referee's earned points as a reward for the referrer
          const referrerPoints = pointsEarned;

          if (referrerPoints > 0) {
            // Update referrer's balance
            const referrerDoc = await getDocument(
              "customers",
              pendingReferral.referrerId,
            );
            if (referrerDoc) {
              const newReferrerPoints =
                (referrerDoc.loyaltyPoints || 0) + referrerPoints;
              await updateDocument("customers", pendingReferral.referrerId, {
              ...
              await addDoc(
                collection(
                  db,
                  `customers/${pendingReferral.referrerId}/pointsHistory`,
                ),
                {
                  type: "earned",
                  points: referrerPoints,
                  description: `Reward for referring ${latestCustomer.name} (Invoice ${invoiceId})`,
                  invoiceId: invoiceId,
              ...
              await markReferralAsCompleted(
                pendingReferral.id,
                referrerPoints,
                invoiceId,
              );
              console.log(
                `✅ Awarded ${referrerPoints} matching points to referrer ${pendingReferral.referrerName}`,
              );
            }
          }
        }
        ...
        try {
          await addDoc(
            collection(db, `customers/${customerId}/pointsHistory`),
            {
              ...
              amount: -(
                pointsDeducted / (globalPointsSettings?.redemptionRate || 20)
              ), // Store rupee value, not points
              description: `Used ${pointsDeducted} points as discount for invoice ${invoiceId}`,
              invoiceId: invoiceId,
              transactionDate: serverTimestamp(),
            },
          );
        } catch (historyError) {
          console.error("Error logging points deduction:", historyError);
          throw historyError;
        }
      }
      ...
      if (pointsEarned > 0) {
        try {
          ...
          console.error("Error logging points earned:", historyError);
