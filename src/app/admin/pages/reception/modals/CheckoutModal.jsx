...
import { DollarSign, Tag, CreditCard, Smartphone, Wallet, X, Printer, Download, MessageCircle, Mail, Coins, Check } from 'lucide-react';
import { generateProfessionalBillPDF, downloadPDF } from '../utils/pdfGenerator';
import { updateDocument, getDocument, getMembership } from '../../../utils/firebaseUtils';
...
const CheckoutModal = ({
  ...
  const [discountType, setDiscountType] = useState('none'); // 'none', 'percentage', 'flat', 'coins', 'membership'
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      try {
        if (visit?.customerId) {
          const latestCustomer = await getDocument('customers', visit.customerId);
          if (latestCustomer) {
            setAvailableLoyaltyPoints(latestCustomer.loyaltyPoints || 0);
            setCustomerData(latestCustomer);
            
            // Fetch membership details
            if (latestCustomer.membershipType && latestCustomer.membershipType !== 'regular') {
              const membership = await getMembership(latestCustomer.membershipType);
  ...
  if (discountType === 'membership' && membershipData) {
  ...
  const handleCompletePayment = async () => {
    try {
      ...
      if (pointsDeducted > 0 || pointsEarned > 0) {
        ...
        await addDoc(collection(db, `customers/${visit.customerId}/pointsHistory`), {
          type: pointsDeducted > 0 && pointsEarned > 0 ? 'adjusted' : (pointsDeducted > 0 ? 'deducted' : 'earned'),
      ...
      const newInvoiceData = {
        ...
        customerPhone: visit.customer?.phone || visit.customer?.contactNo || visit.customerPhone,
        customerEmail: visit.customer?.email || visit.customerEmail || customerData?.email || '',
        items: visit.items,
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ color: '#111827', marginBottom: '0.5rem', fontSize: '1.5rem', fontWeight: '700' }}>Payment Complete</h2>
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <h2 style={{ margin: 0, color: 'white', fontSize: '1.75rem', fontWeight: '800', letterSpacing: '-0.5px' }}>Complete Checkout</h2>
            <p style={{ margin: '0.5rem 0 0 0', color: 'rgba(255,255,255,0.8)', fontSize: '0.875rem', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '1px' }}>
        ...
        <div style={{ padding: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
          ...
          <div>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: '0.75rem', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Customer</p>
                  <p style={{ margin: '0.5rem 0 0 0', fontWeight: '700', color: '#1f2937', fontSize: '1.125rem' }}>{visit.customer?.name}</p>
                ...
                <div>
                  {(() => {
                    const membershipType = customerData?.membershipType || visit.customer?.membershipType || 'regular';
                    ...
                    return (
                      <div style={{
                        ...
                        alignItems: 'center',
                        gap: '0.5rem',
                        padding: isElite ? '0.625rem 1rem' : '0.625rem 0.875rem',
                        background: isElite 
                        ...
                        {isElite ? '👑 Elite' : isMember ? '⭐ Member' : 'Regular'}
                      </div>
                    );
                  })()}
                </div>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <h3 style={{ fontSize: '0.75rem', fontWeight: '700', color: '#374151', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                📦 Items Summary
              </h3>
              <div style={{ 
                background: '#f9fafb',
                ...
                {visit.items?.map((item, idx) => (
                  <div key={idx} style={{ 
                    ...
                    alignItems: 'center',
                    padding: '0.75rem 0',
                    borderBottom: idx < visit.items.length - 1 ? '1px solid #f3f4f6' : 'none'
                  }}>
                    <div>
                      <p style={{ margin: 0, fontWeight: '500', color: '#1f2937' }}>{item.name}</p>
                      <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.75rem', color: '#9ca3af' }}>Qty: {item.quantity}</p>
                    </div>
                    <span style={{ fontWeight: '700', color: '#667eea', fontSize: '0.95rem' }}>₹{(item.price * item.quantity).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem', fontSize: '0.875rem' }}>
                ...
                <span style={{ fontWeight: '600', color: '#78350f' }}>₹{baseTotals.subtotal.toFixed(2)}</span>
              </div>
              {finalDiscount > 0 && (
              ...
              {membershipData && discountType === 'membership' && (
                <div style={{ fontSize: '0.75rem', color: '#b45309', marginBottom: '0.75rem', padding: '0.5rem', background: 'rgba(251, 146, 60, 0.1)', borderRadius: '0.5rem' }}>
                  ✓ {membershipData.name} Member - {membershipData.discountPercentage}% member discount applied
                </div>
              )}
              <div style={{ 
                display: 'flex',
          ...
          <div>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem', marginBottom: '0.75rem' }}>
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (discountType !== 'none') e.currentTarget.style.background = '#e5e7eb';
                  }}
                  onMouseLeave={(e) => {
                    if (discountType !== 'none') e.currentTarget.style.background = '#f3f4f6';
                  }}
                >
                  No Discount
                </button>
                {membershipData && membershipData.discountPercentage > 0 ? (
                  <button
                    ...
                    style={{
                      padding: '0.625rem 0.5rem',
                      background: discountType === 'membership' ? '#fb923c' : '#fef3c7',
                      color: discountType === 'membership' ? 'white' : '#b45309',
                      border: 'none',
                      borderRadius: '0.5rem',
                      fontSize: '0.75rem',
                      fontWeight: '600',
                    ...
                    onMouseEnter={(e) => {
                      if (discountType !== 'membership') e.currentTarget.style.background = '#fed7aa';
                    }}
                    onMouseLeave={(e) => {
                      if (discountType !== 'membership') e.currentTarget.style.background = '#fef3c7';
                    }}
                  >
                ...
                <button
                  ...
                  style={{
                    padding: '0.625rem 0.5rem',
                    background: discountType === 'percentage' ? '#667eea' : '#f3f4f6',
                  ...
                  onMouseEnter={(e) => {
                    if (discountType !== 'percentage') e.currentTarget.style.background = '#e5e7eb';
                  }}
                  onMouseLeave={(e) => {
                    if (discountType !== 'percentage') e.currentTarget.style.background = '#f3f4f6';
                  }}
                >
                  % Discount
                </button>
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (discountType !== 'flat') e.currentTarget.style.background = '#e5e7eb';
                  }}
                  onMouseLeave={(e) => {
                    if (discountType !== 'flat') e.currentTarget.style.background = '#f3f4f6';
                  }}
                >
                  Flat Amount
                </button>
                <button
                  ...
                  style={{
                    padding: '0.625rem 0.5rem',
                    background: discountType === 'coins' ? '#8b5cf6' : '#f3e8ff',
                  ...
                  onMouseEnter={(e) => {
                    if (discountType !== 'coins') e.currentTarget.style.background = '#ede9fe';
                  }}
                  onMouseLeave={(e) => {
                    if (discountType !== 'coins') e.currentTarget.style.background = '#f3e8ff';
                  }}
                >
              ...
              {discountType === 'percentage' || discountType === 'flat' ? (
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.5rem', textAlign: 'center' }}>
                    Available: <span style={{ fontWeight: '600', color: '#1f2937' }}>{availableLoyaltyPoints}</span> Points → Discount: <span style={{ fontWeight: '600', color: '#667eea' }}>₹{(parseFloat(coinsUsed) || 0) / 20}</span>
                  </div>
                </div>
              ) : null}
            </div>
            {/* AMOUNT PAID */}
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '700', color: '#374151', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                💵 Amount Paid
              </label>
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                <span style={{ position: 'absolute', left: '0.75rem', fontSize: '1rem', color: '#6b7280' }}>₹</span>
                <input
                  type="number"
                  value={amountPaid}
                ...
                <button
                  ...
                  onMouseEnter={(e) => e.currentTarget.style.boxShadow = '0 4px 12px rgba(102, 126, 234, 0.4)'}
                  onMouseLeave={(e) => e.currentTarget.style.boxShadow = '0 1px 3px rgba(102, 126, 234, 0.2)'}
              ...
              {amountPaid !== '' && balance !== 0 && (
                <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.875rem', fontWeight: '600', color: balance > 0 ? '#dc2626' : '#10b981' }}>
                ...
                {paymentMethods.map((method) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        padding: '0.75rem',
                        background: paymentMethod === method.id ? method.color : '#f3f4f6',
                        color: paymentMethod === method.id ? 'white' : '#4b5563',
